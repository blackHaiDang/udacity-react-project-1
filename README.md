...
- The user is authenticated on the backend server by a token. This token is initialized the first time the app is run on the frontend server (located on your local machine or on `www.myread.xiaoju.io`), and stored in local.storage. This authentication token can be reset with following command run from the console: `localStorage.removeItem('token')`
